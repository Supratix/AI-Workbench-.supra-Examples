...
- **Source package:** [`../sme_grant_funding_fit_radar.supra`](../sme_grant_funding_fit_radar.supra)
- **Workbench title:** SME Grant Funding Fit Radar Desk
- **Package key:** `sme_grant_funding_fit_radar`
- **Module:** `sme_grant_funding_fit_radar`
- **Vendor:** SupraTix
- **Schema version:** `1`
...
Assess grant-fit signals and produce a shortlist with next evidence and application steps.

## Starter Intake

### SME Grant Funding Fit Radar starter

- **Request:** Paste the source context for SME Grant Funding Fit Radar.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_grant_funding_fit_radar_signals`  | `business_context`                                    | `manual_review` | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_grant_funding_fit_radar_decision` | `business_context`<br>`signal_map`                    | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_grant_funding_fit_radar`          | `business_context`<br>`signal_map`<br>`decision_plan` | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
