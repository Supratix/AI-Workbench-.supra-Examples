...
- **Quellpaket:** [`../sme_quote_to_cash_bottleneck.supra`](../sme_quote_to_cash_bottleneck.supra)
- **Workbench-Titel:** SME Quote-to-Cash Bottleneck Scanner Desk
- **Paket-Key:** `sme_quote_to_cash_bottleneck`
- **Modul:** `sme_quote_to_cash_bottleneck`
- **Anbieter:** SupraTix
- **Schema-Version:** `1`
...
Scan quote-to-cash flow for bottlenecks and create owner-ready fixes.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_quote_to_cash_bottleneck_signals`  | `business_context`                                    | `manual_review` | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_quote_to_cash_bottleneck_decision` | `business_context`<br>`signal_map`                    | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_quote_to_cash_bottleneck`          | `business_context`<br>`signal_map`<br>`decision_plan` | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
