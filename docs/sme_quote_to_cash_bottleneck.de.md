...
Prüft den Quote-to-Cash-Prozess auf Übergabeengpässe, fehlende Daten, Aging-Risiken und verantwortbare Maßnahmen zum Schutz des Umsatzzeitpunkts.

## Starter-Eingabe

### SME Quote-to-Cash Bottleneck Scanner Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Quote-to-Cash Bottleneck Scanner ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_quote_to_cash_bottleneck_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_quote_to_cash_bottleneck_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_quote_to_cash_bottleneck`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
