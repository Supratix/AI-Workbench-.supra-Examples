...
- **Source package:** [`../sme_margin_leak_detector.supra`](../sme_margin_leak_detector.supra)
- **Workbench title:** SME Margin Leak Detector Desk
- **Package key:** `sme_margin_leak_detector`
- **Module:** `sme_margin_leak_detector`
- **Vendor:** SupraTix
- **Schema version:** `1`
...
Identify margin leakage signals and create a practical profit-protection action plan.

## Starter Intake

### SME Margin Leak Detector starter

- **Request:** Paste the source context for SME Margin Leak Detector.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_margin_leak_detector_signals`  | `business_context`                                    | `manual_review` | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_margin_leak_detector_decision` | `business_context`<br>`signal_map`                    | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_margin_leak_detector`          | `business_context`<br>`signal_map`<br>`decision_plan` | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
