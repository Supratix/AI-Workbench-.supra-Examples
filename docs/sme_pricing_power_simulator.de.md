...
Bewertet Preissetzungsmacht anhand von Nachfrage-, Wettbewerbs-, Kosten- und Kundensignalen und schlägt Szenarien, Guardrails und Review-Trigger vor.

## Starter-Eingabe

### SME Pricing Power Simulator Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Pricing Power Simulator ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_pricing_power_simulator_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_pricing_power_simulator_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_pricing_power_simulator`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
