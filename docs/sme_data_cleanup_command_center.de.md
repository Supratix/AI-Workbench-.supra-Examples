...
Erstellt einen praxisnahen Datenbereinigungsplan mit Problemkategorien, Qualitätsregeln, Verantwortlichen, Validierungschecks und Umsetzungsreihenfolge.

## Starter-Eingabe

### SME Data Cleanup Command Center Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Data Cleanup Command Center ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_data_cleanup_command_center_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_data_cleanup_command_center_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_data_cleanup_command_center`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
