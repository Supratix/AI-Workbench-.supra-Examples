...
- **Quellpaket:** [`../sme_data_cleanup_command_center.supra`](../sme_data_cleanup_command_center.supra)
- **Workbench-Titel:** SME Data Cleanup Command Center Desk
- **Paket-Key:** `sme_data_cleanup_command_center`
- **Modul:** `sme_data_cleanup_command_center`
- **Anbieter:** SupraTix
- **Schema-Version:** `1`
...
Create a practical data cleanup plan with quality rules, owners, and validation checks.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_data_cleanup_command_center_signals`  | `business_context`                                    | `manual_review` | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_data_cleanup_command_center_decision` | `business_context`<br>`signal_map`                    | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_data_cleanup_command_center`          | `business_context`<br>`signal_map`<br>`decision_plan` | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
