...
Bereitet einen menschlichen, aber klaren Mahnplan mit Kundenkontext, Nachrichtenvorlagen, Eskalationspfad und Zahlungsrisikosignalen vor.

## Starter-Eingabe

### SME Late Payment Collector Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Late Payment Collector ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_late_payment_collector_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_late_payment_collector_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_late_payment_collector`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
