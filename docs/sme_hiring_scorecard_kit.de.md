...
Erstellt eine Rollen-Scorecard mit Erfolgszielen, Interview-Signalen, Entscheidungsrubrik, Evidenzfragen und Hiring-Risikohinweisen für KMU-Teams.

## Starter-Eingabe

### SME Hiring Scorecard Kit Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Hiring Scorecard Kit ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_hiring_scorecard_kit_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_hiring_scorecard_kit_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_hiring_scorecard_kit`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
