...
- **Quellpaket:** [`../sme_warranty_root_cause_radar.supra`](../sme_warranty_root_cause_radar.supra)
- **Workbench-Titel:** SME Warranty Root Cause Radar Desk
- **Paket-Key:** `sme_warranty_root_cause_radar`
- **Modul:** `sme_warranty_root_cause_radar`
- **Anbieter:** SupraTix
- **Schema-Version:** `1`
...
Analyze warranty issues and propose root-cause and containment actions.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_warranty_root_cause_radar_signals`  | `business_context`                                    | `manual_review` | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_warranty_root_cause_radar_decision` | `business_context`<br>`signal_map`                    | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_warranty_root_cause_radar`          | `business_context`<br>`signal_map`<br>`decision_plan` | `manual_review` | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
