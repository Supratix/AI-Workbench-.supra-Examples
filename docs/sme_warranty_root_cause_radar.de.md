...
Analysiert Garantieprobleme und schlägt Root-Cause-Hypothesen, Containment-Maßnahmen, Evidenzlücken, Kundenwirkung und Präventionschecks vor.

## Starter-Eingabe

### SME Warranty Root Cause Radar Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Warranty Root Cause Radar ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_warranty_root_cause_radar_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_warranty_root_cause_radar_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_warranty_root_cause_radar`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
