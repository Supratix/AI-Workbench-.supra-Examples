...
Erstellt eine Bid/No-Bid-Empfehlung für Ausschreibungen mit Fit-Bewertung, Evidenzlücken, Lieferrisiken, Governance-Hinweisen und nächsten Maßnahmen.

## Starter-Eingabe

### SME Tender No-Bid Gate Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Tender No-Bid Gate ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_tender_no_bid_gate_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_tender_no_bid_gate_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_tender_no_bid_gate`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
