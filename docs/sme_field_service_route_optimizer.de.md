...
Wandelt Serviceaufträge, Techniker-Kapazitäten, Reiserestriktionen und Prioritätsregeln in ein Routenoptimierungs-Briefing für Außendienstteams um.

## Starter-Eingabe

### SME Field Service Route Optimizer Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Field Service Route Optimizer ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_field_service_route_optimizer_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_field_service_route_optimizer_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_field_service_route_optimizer`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
