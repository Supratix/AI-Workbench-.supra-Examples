...
Entwirft ein Angebotsbriefing mit Kundenzielen, Scope, Annahmen, Liefergegenständen, Ausschlüssen, Risiken und nächster Handlungssprache.

## Starter-Eingabe

### SME Proposal Drafter Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Proposal Drafter ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_proposal_drafter_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_proposal_drafter_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_proposal_drafter`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
