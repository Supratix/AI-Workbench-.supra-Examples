...
- **Quellpaket:** [`../sme_churn_rescue_desk.supra`](../sme_churn_rescue_desk.supra)
- **Workbench-Titel:** SME Churn Rescue Desk Desk
- **Paket-Key:** `sme_churn_rescue_desk`
- **Anbieter:** SupraTix
- **Schemaversion:** `1`
- **Spalten:** 4
- **Workflows:** 1
...
Erkennt Abwanderungssignale aus Kundenkontext und erstellt einen praktikablen Rettungsplan mit Ursachen, Verantwortlichenaktionen und Review-Triggern.

## Starter-Eingabe

### SME Churn Rescue Desk Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Churn Rescue Desk ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_churn_rescue_desk_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_churn_rescue_desk_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_churn_rescue_desk`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
