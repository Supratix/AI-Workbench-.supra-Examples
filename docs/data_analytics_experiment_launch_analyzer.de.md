...
Wandelt Experiment- oder Launch-Notizen in eine entscheidungsreife Analytics-Prüfung mit Hypothesenchecks, Ergebnissignalen, Risiken und offenen Instrumentierungsbedarfen um.

## Starter-Eingabe

### Data Analytics Experiment And Launch Analyzer Starter

- **Anfrage:** Fügen Sie den Quellkontext für Data Analytics Experiment And Launch Analyzer ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `data_analytics_experiment_launch_analyzer_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `data_analytics_experiment_launch_analyzer_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `data_analytics_experiment_launch_analyzer`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
