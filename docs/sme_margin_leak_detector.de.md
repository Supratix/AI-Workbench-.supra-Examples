...
Identify margin leakage from pricing, discounting, cost, and delivery signals, then create a practical profit-protection action plan.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_margin_leak_detector_signals`  | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_margin_leak_detector_decision` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_margin_leak_detector`          | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
