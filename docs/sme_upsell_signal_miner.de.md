...
Findet Upsell-Signale im Account-Kontext und übersetzt sie in accountspezifische Angebote, Timing-Hinweise, Risikoflaggen und nächste Maßnahmen.

## Starter-Eingabe

### SME Upsell Signal Miner Starter

- **Anfrage:** Fügen Sie den Quellkontext für SME Upsell Signal Miner ein.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_upsell_signal_miner_signals`  | ja      | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_upsell_signal_miner_decision` | ja      | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_upsell_signal_miner`          | ja      | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
