...
Convert cyber hygiene observations into a prioritized remediation board with risk level, responsible owners, due dates, and verification checks.
...
| 2 | `signal_map`       | Signal map       | `ai_tool`  | `sme_cyber_hygiene_action_board_signals`  | yes    | `summary`<br>`signals`<br>`constraints`<br>`assumptions`<br>`risks`<br>`evidence_gaps` |
| 3 | `decision_plan`    | Decision plan    | `ai_tool`  | `sme_cyber_hygiene_action_board_decision` | yes    | `summary`<br>`decision`<br>`actions`<br>`metrics`<br>`risks`<br>`evidence_gaps`        |
| 4 | `execution_brief`  | Execution brief  | `shortcut` | `sme_cyber_hygiene_action_board`          | yes    | `summary`<br>`decision`<br>`actions`<br>`risks`<br>`evidence_gaps`                     |
